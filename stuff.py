...
def define_state():
	...
	if  (checkpoint == 1 or checkpoint == 2 ):                           # these checkpoints only require to check wall on right
		if  (regions_['fright'] > 0.5  and regions_['bright'] > 0.5) : 
			delay(1.25)
			change_state(2)
		...
		if  (regions_['fleft'] > 0.5 and regions_['bleft'] > 0.5) : 
			delay(1.25)
			change_state(2)
...
def control_loop():
	...
	while not rospy.is_shutdown() :
		...
		pub.publish(msg)
		print("x = {} z = {} State = {} c = {}".format(msg.linear.x,msg.angular.z,state,checkpoint))
